...
public class LevelGenerator : MonoBehaviour
{
    ...
    {
        boospart.SetActive(true);
        timepart.SetActive(true);
        stageCleared = true;
        SoundManager.Instance.Play_StageClear();
        int currentMaxStage;
        PlayerPrefs.SetInt("clearLv", currentStage);
        int lv = PlayerPrefs.GetInt("clearLv",0);
        int lvMax = PlayerPrefs.GetInt("CurMaxLv",1);
        if (lvMax <= lv)
        {
            PlayerPrefs.SetInt("CurMaxLv", PlayerPrefs.GetInt("clearLv"));
        }
        ...
        PlayerPrefs.Save();
        GameObject.Find("StageText").GetComponent<Text>().text = "关卡 " + currentStage.ToString();
