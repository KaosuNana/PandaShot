...
public class DailyRewards : MonoBehaviour
{
    ...
    void Update()
    {
        reci.transform.GetChild(0).GetChild(3).gameObject.SetActive(EventSystem.current.currentSelectedGameObject == reci.gameObject);
        if (holder.transform.localScale.x == 1 && !AdsManager.Instance.IsAc)
        {
            EventSystem.current.SetSelectedGameObject(null);
            EventSystem.current.SetSelectedGameObject(reci.gameObject);
        }
        if(holder.transform.localScale.x != 1 && EventSystem.current.currentSelectedGameObject == reci.gameObject)
            SetFocus();
    }
