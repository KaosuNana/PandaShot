...
public class Tut_Bullet : MonoBehaviour {
	...
	void Update () 
	{
		if(available && initialized)
		{
			available = false;
			initialized = false;
			transform.position = Tut_PlaneManager.Instance.firePosition.position;
			transform.GetChild(0).GetComponent<Animation>().Play();
			transform.GetChild(1).GetComponent<Animation>().Play();
		}
		if(!available && transform.position.y > Tut_PlaneManager.Instance.boundUp)
		{
			available = true;
