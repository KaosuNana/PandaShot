...
public class Tut_SideBullet : MonoBehaviour {
	
	public bool available = true;
	public bool initialized = false;
	public static int speed = 35;//25, pa 45
	...
	void Update () 
	{
		if(available && initialized)
		{
			available = false;
			initialized = false;
			transform.position = Tut_PlaneManager.Instance.sideFirePosition.position;
			transform.GetChild(0).GetComponent<Animation>().Play();
			transform.GetChild(1).GetComponent<Animation>().Play();
		}
		if(!available && (transform.GetChild(0).position.x < Tut_PlaneManager.Instance.boundLeft && transform.GetChild(1).position.x > Tut_PlaneManager.Instance.boundRight))
		{
			available = true;
