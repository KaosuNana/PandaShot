...
public class PlayButton : MonoBehaviour
{
    ...
    void Start()
    {
        if (PlayerPrefs.HasKey("MaxStage"))
        {
            ...
            int lv = PlayerPrefs.GetInt("CurMaxLv", 1);
            if (cms == ghe)
            {
                if (cms == fsd)
                {
                    if (PlayerPrefs.GetInt("clearLv", 0) == 0)
                        currentMaxStage = 1;
                    else
                    {
                        currentMaxStage = lv + 1;
        ...
        if (currentMaxStage > 25)
        {
            ...
            for (int i = 1; i < 25; i++)
            {
                Checkpoints.transform.GetChild(i).gameObject.GetComponent<Button>().interactable = true;
                Checkpoints.transform.GetChild(i).gameObject.GetComponent<Image>().color = Color.white;
                Checkpoints.transform.GetChild(i).GetChild(1).gameObject.SetActive(true);
                Checkpoints.transform.GetChild(i).GetChild(0).GetComponent<Text>().color = Color.white;
                Checkpoints.transform.GetChild(i).GetChild(2).gameObject.SetActive(false);
                Checkpoints.transform.GetChild(i).GetChild(1).GetComponent<Text>().text = (checkpointStartPrice * i).ToString();
            }
    ...
    public void PlayButtonPressed()
    {
        ...
        EventSystem.current.SetSelectedGameObject(lvbtn.gameObject);
        //		}
        //UPDATE!!!!!
    ...
    public void CloseCheckpointMenu()
    {
        ...
        EventSystem.current.SetSelectedGameObject(null);
        EventSystem.current.SetSelectedGameObject(this.gameObject.transform.GetComponent<Button>().gameObject);
        //UPDATE!!!!!
        //GameObject.Find("CheckpointsScreen/NativeAdHolder").GetComponent<FacebookNativeAd>().CancelLoading();
        //GameObject.Find("CheckpointsScreen/NativeAdHolder/AnimationHolder").GetComponent<Animator>().Play("NativeAdsDeparting");
