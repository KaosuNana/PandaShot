...
public class BossPlane : MonoBehaviour {
	...
	void Start () 
	{
		...
		{
			if(LevelGenerator.currentStage > 9)
				smallWingTurretHealth += factorX * PandaPlane.mainGunLvl + (int)(factorY*LevelGenerator.currentStage/9f);
			else
				smallWingTurretHealth += factorX * PandaPlane.mainGunLvl + factorY;//*PandaPlane.mainGunLvl;
		}
		if(tailGunHealth != 0)
		{
			if(LevelGenerator.currentStage > 9)
				tailGunHealth += factorX * PandaPlane.mainGunLvl + (int)(factorY*LevelGenerator.currentStage/9f);
			else
				tailGunHealth += factorX * PandaPlane.mainGunLvl + factorY;//*PandaPlane.mainGunLvl;
		}
		if(wingTurretSmallHealth != 0)
		{
			if(LevelGenerator.currentStage > 9)
				wingTurretSmallHealth += factorX * PandaPlane.mainGunLvl + (int)(factorY*LevelGenerator.currentStage/9f);
			else
				wingTurretSmallHealth += factorX * PandaPlane.mainGunLvl + factorY;//*PandaPlane.mainGunLvl;
		}
		if(wingTurretLargeHealth != 0)
		{
			if(LevelGenerator.currentStage > 9)
				wingTurretLargeHealth += factorX * PandaPlane.mainGunLvl + (int)(factorY*LevelGenerator.currentStage/9f);
			else
				wingTurretLargeHealth += factorX * PandaPlane.mainGunLvl + factorY;//*PandaPlane.mainGunLvl;
		}
		if(wingLaserGunSmallHealth != 0)
		{
			if(LevelGenerator.currentStage > 9)
				wingLaserGunSmallHealth += factorX * PandaPlane.mainGunLvl + (int)(factorY*LevelGenerator.currentStage/9f);
			else
				wingLaserGunSmallHealth += factorX * PandaPlane.mainGunLvl + factorY;//*PandaPlane.mainGunLvl;
		}
		if(wingLaserGunLargeHealth != 0)
		{
			if(LevelGenerator.currentStage > 9)
				wingLaserGunLargeHealth += factorX * PandaPlane.mainGunLvl + (int)(factorY*LevelGenerator.currentStage/9f);
			else
				wingLaserGunLargeHealth += factorX * PandaPlane.mainGunLvl + factorY;//*PandaPlane.mainGunLvl;
		}
		if(mainGunHealth != 0)
		{
			if(LevelGenerator.currentStage > 9)
				mainGunHealth += (int)(factorX * PandaPlane.mainGunLvl + (int)(factorY*1.5f*LevelGenerator.currentStage/9f));
			else
				mainGunHealth += (int)(factorX * PandaPlane.mainGunLvl + factorY*1.5f);//*PandaPlane.mainGunLvl;
		}
...
//		}
	...
	IEnumerator AdjustCameraSpeedForBoss()
	{
		...
		while(GameManager.Instance.speed != bgMovingSpeed)
		{
			yield return null;
			GameManager.Instance.speed = Mathf.MoveTowards(GameManager.Instance.speed, bgMovingSpeed, 0.2f);
		}
