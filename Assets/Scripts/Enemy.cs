...
public class Enemy : MonoBehaviour {
	...
	void Start () {
		...
		damage = (int)((LevelGenerator.currentStage-1)*20 + (float)PandaPlane.healthValues[1]*0.02f);
	}
...
//		TargetAndFire();
	...
	public void InitializeEnemyHealth()
	{
		...
		{
			health=healthValuesMainGunForAirBasic[PandaPlane.mainGunLvl];
			health+=healthValuesWingGunLvlForAirBasic[PandaPlane.wingGunLvl];
			health+=healthValuesSideGunLvlForAirBasic[PandaPlane.sideGunLvl];
			health+=healthValuesBombLvlForAirBasic[PandaPlane.bombLvl];
			health+=healthValuesTeslaLvlForAirBasic[PandaPlane.teslaLvl];
			health+=healthValuesLaserLvlForAirBasic[PandaPlane.laserLvl];
			health+=healthValuesBladesLvlForAirBasic[PandaPlane.bladesLvl];
		...
		{
			health=healthValuesMainGunForAirIntermediate[PandaPlane.mainGunLvl];
			health+=healthValuesWingGunLvlForAirIntermediate[PandaPlane.wingGunLvl];
			health+=healthValuesSideGunLvlForAirIntermediate[PandaPlane.sideGunLvl];
			health+=healthValuesBombLvlForAirIntermediate[PandaPlane.bombLvl];
			health+=healthValuesTeslaLvlForAirIntermediate[PandaPlane.teslaLvl];
			health+=healthValuesLaserLvlForAirIntermediate[PandaPlane.laserLvl];
			health+=healthValuesBladesLvlForAirIntermediate[PandaPlane.bladesLvl];
			...
			if(currentMaxStage>1)
			{
				...
				else if(AirIntermediateReference==AirIntermediate.AirIntermediate14RightUpToCenterPauseFireToLeftDown)
				{
					health+=60;
					damage+=60;
				}
		...
		{
			health=healthValuesMainGunForAirAdvanced[PandaPlane.mainGunLvl];
			health+=healthValuesWingGunLvlForAirAdvanced[PandaPlane.wingGunLvl];
			health+=healthValuesSideGunLvlForAirAdvanced[PandaPlane.sideGunLvl];
			health+=healthValuesBombLvlForAirAdvanced[PandaPlane.bombLvl];
			health+=healthValuesTeslaLvlForAirAdvanced[PandaPlane.teslaLvl];
			health+=healthValuesLaserLvlForAirAdvanced[PandaPlane.laserLvl];
			health+=healthValuesBladesLvlForAirAdvanced[PandaPlane.bladesLvl];
		...
		{
			health=healthValuesMainGunForGroundAndWaterStatic[PandaPlane.mainGunLvl];
			health+=healthValuesWingGunLvlForGroundAndWaterStatic[PandaPlane.wingGunLvl];
			health+=healthValuesSideGunLvlForGroundAndWaterStatic[PandaPlane.sideGunLvl];
			health+=healthValuesBombLvlForGroundAndWaterStatic[PandaPlane.bombLvl];
			health+=healthValuesTeslaLvlForGroundAndWaterStatic[PandaPlane.teslaLvl];
			health+=healthValuesLaserLvlForGroundAndWaterStatic[PandaPlane.laserLvl];
			health+=healthValuesBladesLvlForGroundAndWaterStatic[PandaPlane.bladesLvl];
		...
		{
			health=healthValuesMainGunForGroundAndWaterMobile[PandaPlane.mainGunLvl];
			health+=healthValuesSideGunLvlForGroundAndWaterMobile[PandaPlane.wingGunLvl];
			health+=healthValuesSideGunLvlForGroundAndWaterMobile[PandaPlane.sideGunLvl];
			health+=healthValuesBombLvlForGroundAndWaterMobile[PandaPlane.bombLvl];
			health+=healthValuesTeslaLvlForGroundAndWaterMobile[PandaPlane.teslaLvl];
			health+=healthValuesLaserLvlForGroundAndWaterMobile[PandaPlane.laserLvl];
			health+=healthValuesBladesLvlForGroundAndWaterMobile[PandaPlane.bladesLvl];
			if(currentMaxStage>1)
			{
...
//	}
	...
	public void TakeDamage(int damage)
	{
		if(health-damage<0)
		{
			...
			CoinsOnDeath();
			PandaPlane.enemiesKilledFromLastCollectable++;
			if(PandaPlane.collectableNumber<=PandaPlane.enemiesKilledFromLastCollectable)
			{
				PandaPlane.enemiesKilledFromLastCollectable=0;
				if(!PlayerPrefs.HasKey("MaxStage"))
				{
					PandaPlane.collectableNumber = Random.Range(8,16);
				}
				else
				{
					PandaPlane.collectableNumber = Random.Range(15,26);
				}
				CollectableOnDeath();
			}
	...
	//			gameObject.SetActive(false);
				...
				TakeDamage(PandaPlane.mainGunDamage);//25
	//			TakeDamage(30);
	//			TakeDamage(5);
			}
			...
			else if(col.tag.Equals("Laser"))
			{
				...
				StartCoroutine(DoContinuousDamage(PandaPlane.laserDamage));
			}
			else if(col.tag.Equals("Tesla"))
			{
				continuousDamage = true;
				StartCoroutine(DoContinuousDamage(PandaPlane.teslaDamage));
			}
...
//		}
	...
	void CollectableOnDeath()
	{
		do
		{
			PandaPlane.generatedCollectable = Random.Range(0, PandaPlane.collectables.Count);
		}
		while(PandaPlane.generatedCollectable==PandaPlane.lastGeneratedCollectable);
		
		PandaPlane.lastGeneratedCollectable=PandaPlane.generatedCollectable;
		...
		if(!GameManager.Instance.bossTime)
		{
			...
			switch(PandaPlane.collectables[PandaPlane.generatedCollectable])
			{
